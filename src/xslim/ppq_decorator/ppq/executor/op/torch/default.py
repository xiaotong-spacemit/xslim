...
def Conv_forward(op: Operation, values: List[torch.Tensor], ctx: TorchBackendContext = None, **kwargs) -> torch.Tensor:
    ...
    if ndim in {2, 3}:
        ...
        if isinstance(torch_pads, list) and len(torch_pads) == 2:
            ...
            torch_pads = 0
        output = F.conv1d(
            input=x, weight=w, bias=b, groups=groups, padding=torch_pads, dilation=dilation, stride=stride
        )
    ...
    # conv - 2d
        ...
        if XQUANT_EVAL_Q4:
            ...
            x_quantize = quantize_channel_blockwise(x.reshape(batch, ic, -1).permute(0, 2, 1).reshape(-1, ic)).to(
                torch.float16
            )
            ...
            output = F.conv2d(
                input=x, weight=w, bias=b, groups=groups, padding=onnx_pads, dilation=dilation, stride=stride
            )
...
def MaxPool2d_forward(
    ...
    if ndim == 5:
        ...
        if isinstance(torch_pads, list) and len(torch_pads) != 3:
            ...
            torch_pads = 0
        output = F.max_pool3d(
            x, kernel_size=kernel_size, padding=torch_pads, dilation=dilation, stride=stride, ceil_mode=ceil_mode
        )
        ...
        output = F.max_pool2d(
            x, kernel_size=kernel_size, padding=onnx_pads, dilation=dilation, stride=stride, ceil_mode=ceil_mode
        )
...
def BatchNormalization_forward(
    op: Operation, values: List[torch.Tensor], ctx: TorchBackendContext = None, **kwargs
) -> torch.Tensor:
...
def GatherND_forward(
    ...
    # indices reshape
    reshaped_indices = indices.reshape(*shape_i, -1, num_idx)  # shape [i_1, ..., i_b, J, 1]
    # indices tensordot, expand the last dim in indices
    strides = torch.tensor(
        [reduce(operator.mul, shape_idx[i + 1 :], 1) for i in range(num_idx)],
        device=input_data.device,
        dtype=torch.float,
    )
    merged_indices = torch.tensordot(reshaped_indices.float(), strides, 1)  # shape [i_1, ..., i_b, J]

    # indices expand
    expanded_indices = (
        merged_indices.reshape(*merged_indices.shape, *([1] * num_k)).expand(*merged_indices.shape, *shape_k).long()
    )
...
def _reduce_prod(input_value: torch.Tensor, dim, keepdim: bool):
    if isinstance(dim, int):
        dim = (dim,)
    dim = tuple(axis if axis >= 0 else axis + input_value.dim() for axis in dim)
    output = input_value
...
def ReduceMean_forward(op: Operation, values: List[torch.Tensor], ctx: TorchBackendContext = None, **kwargs):
    ...
    if noop or input_value.numel() == 0:
        return input_value
    if dim is None:
        return _reshape_reduce_all_output(torch.mean(input_value), input_value, keepdim)
    return torch.mean(input_value, dim=dim, keepdim=keepdim)


def ReduceSum_forward(op: Operation, values: List[torch.Tensor], ctx: TorchBackendContext = None, **kwargs):
    input_value, dim, keepdim, noop = _get_reduce_inputs(op, values, min_input_opset=13)
    if noop or input_value.numel() == 0:
        return input_value
    if dim is None:
        return _reshape_reduce_all_output(torch.sum(input_value), input_value, keepdim)
    return torch.sum(input_value, dim=dim, keepdim=keepdim)
...
def MatMul_forward(
    ...
    if XQUANT_EVAL_Q4:
        ...
        val_rhs_quant = (
            quantize_groupwise(val_rhs).reshape(-1, N, K).permute(0, 2, 1).reshape(val_rhs_shape).to(torch.float16)
        )
...
def ReduceL2_forward(op: Operation, values: List[torch.Tensor], ctx: TorchBackendContext = None, **kwargs):
    ...
    if input_value.numel() == 0:
        return input_value
    if dim is None:
        output = torch.linalg.vector_norm(input_value)
        return _reshape_reduce_all_output(output, input_value, keepdim)
    return torch.linalg.vector_norm(input_value, dim=dim, keepdim=keepdim)
        ...
        if has_bias:
            ...
            forward_bias_2 = torch.cat(
                [
                    ...
                    b[0, hidden_size * 5 : hidden_size * 6],
                ]
            ).contiguous()
            ...
            if has_bias:
                ...
                reverse_bias_2 = torch.cat(
                    [
                        ...
                        b[1, hidden_size * 5 : hidden_size * 6],
                    ]
                ).contiguous()
